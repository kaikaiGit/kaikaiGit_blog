...
const degrees = ref(57.296);
const cursorSize = ref(0);

const move = (event, root) => {
  previousPointerX.value = position.pointerX;
  previousPointerY.value = position.pointerY;
  position.pointerX = event.pageX + root.getBoundingClientRect().x;
  position.pointerY = event.pageY + root.getBoundingClientRect().y;
  position.distanceX = previousPointerX.value - position.pointerX;
  position.distanceY = previousPointerY.value - position.pointerY;
  position.distance = Math.sqrt(position.distanceY ** 2 + position.distanceX ** 2);

  curzr.value.style.transform = `translate3d(${position.pointerX}px, ${position.pointerY}px, 0)`;
...
const rotate = (position) => {
  let unsortedAngle = Math.atan(Math.abs(position.distanceY) / Math.abs(position.distanceX)) * degrees.value;
  let modAngle;
...
onMounted(() => {
  if (!/Android|webOS|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.test(navigator.userAgent)) {
    ...
    document.body.addEventListener('mousemove', (event) => {
      move(event, document.body);
    });
  } else {
    curzr.value.parentNode.removeChild(curzr.value);
  }
});
</script>
